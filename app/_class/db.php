...
class db {
    ...
    public function query($query) {
		...
		if ($this->query = $this->connection->prepare($query)) {
            ...
            $this->query->execute();
           	if ($this->query->errno) {
				$this->error('Unable to process MySQL query (check your params) - ' . $this->query->error . ' (' . $query . ')');
           	}
	...
	public function lockTables(array $tables){
		$str = implode(" WRITE, ", $tables) . " WRITE";
		$this->querySimple("LOCK TABLES {$str}");
	}
	
	public function unlockTables(){	
		$this->querySimple("UNLOCK TABLES");
	}
